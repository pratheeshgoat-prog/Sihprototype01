...
st.sidebar.divider()
st.sidebar.header("📍 Dam Locator")

dam_lat = st.sidebar.number_input(
    "Dam latitude", value=float(breach_lat), format="%.6f"
)
dam_lon = st.sidebar.number_input(
    "Dam longitude", value=float(breach_lon), format="%.6f"
)

if st.sidebar.button("📍 Use demo dam location", use_container_width=True):
    st.session_state["dam_lat"] = float(breach_lat)
    st.session_state["dam_lon"] = float(breach_lon)
    st.rerun()

dam_lat = float(st.session_state.get("dam_lat", dam_lat))
dam_lon = float(st.session_state.get("dam_lon", dam_lon))

# The breach follows the selected dam locator.
breach_lat = dam_lat
breach_lon = dam_lon

view_mode = st.sidebar.radio(
    "Digital Twin View",
    ["2D Flood Map", "3D Digital Twin"],
)
...
if view_mode == "2D Flood Map":
    ...
    folium.Marker(
        ...
        tooltip=f"🔴 DAM — T+{t_min} min",
        popup=(
            f"<b>Dam / Breach Locator</b><br>"
            f"Latitude: {breach_lat:.6f}<br>"
            f"Longitude: {breach_lon:.6f}<br>"
    ...
    st_folium(
        ...
        width=1200,
        height=620,
        key=f"flood-map-{t_min}-{breach_lat:.6f}-{breach_lon:.6f}",
    )

else:
    st.subheader("🏙️ 3D Digital Twin — Live Dam-Break Animation")

    # Downsample the DEM for browser performance.
    step = 4
    terrain = pd.DataFrame({
        "lon": lon_grid[::step, ::step].ravel(),
        "lat": lat_grid[::step, ::step].ravel(),
        "elevation": elevation[::step, ::step].ravel(),
        "depth": flood_depth[::step, ::step].ravel(),
    })

    terrain["water_top"] = terrain["elevation"] + terrain["depth"]
    ...
    terrain_layer = pdk.Layer(
        ...
        elevation_scale=5,
        radius=11,
        get_fill_color=[115, 115, 115, 100],
        pickable=True,
    )

    # Water is physically drawn above the terrain, so the viewer can see
    # the flood layer advancing as the simulated minute changes.
    water = terrain[terrain["depth"] > 0.03].copy()
    ...
    water_layer = pdk.Layer(
        ...
        elevation_scale=5,
        radius=12,
        get_fill_color=[20, 130, 255, 185],
        pickable=True,
    )

    # Buildings become taller 3D objects and change height based on risk.
    twin_buildings = display_buildings.copy()
    twin_buildings["height"] = (
        25 + twin_buildings["flood_depth_m"].clip(0, 2) * 25
    )

    risk_rgba = {
        "Safe": [50, 180, 80, 230],
        "Low": [180, 180, 180, 230],
        "Medium": [255, 165, 0, 240],
        "High": [235, 60, 40, 245],
        "Critical": [120, 0, 0, 250],
    }
    twin_buildings["rgba"] = twin_buildings["risk_level"].map(risk_rgba)
    ...
    road_points = pd.DataFrame(road_points)

    if not road_points.empty:
        road_points["rgba"] = road_points["status"].map({
            "Clear": [60, 60, 60, 230],
            "Caution": [255, 165, 0, 240],
            "Impassable": [230, 30, 30, 250],
        })
    ...
    # Dam locator: a large 3D red column + text label.
    dam_df = pd.DataFrame([{
        "lon": breach_lon,
        "lat": breach_lat,
        "height": 80,
        "label": f"DAM / BREACH — T+{t_min} min",
    }])
    ...
    deck = pdk.Deck(
        ...
        tooltip={
            "html": "<b>{label}</b><br/>Depth: {depth} m",
    ...
    st.pydeck_chart(
        ...
        key=f"3d-twin-{t_min}-{breach_lat:.6f}-{breach_lon:.6f}",
    )

    st.info(
        f"🔴 Dam locator: {breach_lat:.6f}, {breach_lon:.6f}  |  "
        f"🌊 Water front simulation: T+{t_min} min"
    )
