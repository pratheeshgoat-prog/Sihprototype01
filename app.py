...
if "monitoring_data" not in st.session_state:
    ...
    n_readings = 30
    base_level = 105
    trend = np.linspace(0, 12, n_readings)  # rising trend toward breach threshold
...
if slope > 0:
    ...
    hours_per_reading = 1  # demo assumption: 1 reading per hour
    hours_to_breach = max(readings_to_breach * hours_per_reading, 0)
else:
...
st.caption("Prediction method: linear trend extrapolation on recent readings — a simple illustrative heuristic, not a structural/geotechnical failure model.")
...
if breach_reached:
    ...
    # ... your existing simulate_flood_at_time / simulate_flood call goes here,
    # feeding water_level = current_level instead of the manual slider default
    # your existing buildings["risk_level"] classification (Safe/Low/Medium/High/Critical)
    # and roads_gdf["status"] logic runs unchanged on this flood_depth

    risk_summary = buildings["risk_level"].value_counts()
    st.write("**Buildings by risk category:**")
    st.bar_chart(risk_summary)

    # your existing pydeck 3D block goes here, driven by this flood_depth
else:
    st.info("Dam stable. 3D flood visualization will appear once breach threshold is reached.")
# =========================================
